import { addDays, daysBetween, getWeeks, monday, parseDate } from '../lib/calendar'
...
export async function readCapacity(range: DateRange, signal?: AbortSignal): Promise<CapacityData> {
    ...
    people: people.map(person => ({
      ...
      allocations: weeks.map(week => {
        ...
        for (let day = week.from; day <= week.to; day = addDays(day, 1)) {
          ...
          const variation = person.id === 4 || person.id === 5 || person.id === 9
            ? 0 : ((weekIndex + person.id) % 3 + 3) % 3 * 0.5
...
export async function updatePerson(id: number, weeklyHours: number): Promise<Person> {
  await delay(450)
