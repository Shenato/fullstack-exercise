...
export function useCapacity(range: DateRange, refreshSeconds: number | false) {
  ...
  const updateCaches = (person: Person) => client.setQueriesData<CapacityData>(
    { queryKey: ['capacity'] },
    previous => previous && {
      ...previous,
      people: previous.people.map(row => row.id === person.id ? { ...row, ...person } : row),
    },
  )
  const mutation = useMutation({
    ...
    onMutate: async person => {
      ...
      const snapshots = client.getQueriesData<CapacityData>({ queryKey: ['capacity'] })
      updateCaches(person)
      return { snapshots }
    },
    ...
    onError: (_error, person, context) => {
      for (const [key, data] of context?.snapshots ?? []) {
        ...
        client.setQueryData<CapacityData>(key, current => current && {
          ...current,
          people: current.people.map(row => row.id === person.id ? { ...row, weeklyHours: original.weeklyHours } : row),
        })
      }
    },
    onSettled: () => { void client.invalidateQueries({ queryKey: ['capacity'] }) },
  })
