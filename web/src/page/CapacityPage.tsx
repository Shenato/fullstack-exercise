...
import type {
  ...
  DisplayUnit,
  Person,
  SprintSettings as Settings,
} from "../types/capacity";

const SprintSettings = lazy(() =>
  import("../container/SprintSettings").then((module) => ({
    default: module.SprintSettings,
  })),
);
const WeeklyHoursEditor = lazy(() =>
  import("../container/WeeklyHoursEditor").then((module) => ({
    default: module.WeeklyHoursEditor,
  })),
);
...
export function CapacityPage() {
  ...
  const { query, mutation } = useCapacity(
    range,
    automatic ? seconds + jitter : false,
  );
  const weeks = query.data?.weeks ?? getWeeks(range);
  const people = (query.data?.people ?? []).filter(
    (person) =>
      person.name.toLowerCase().includes(search.trim().toLowerCase()) &&
      (!overOnly ||
        person.allocations.some(
          (hours, index) =>
            hours > weekCapacity(person.weeklyHours, weeks[index].workdays),
        )),
  );
  const allocated = people.reduce(
    (sum, person) =>
      sum + person.allocations.reduce((total, hours) => total + hours, 0),
    0,
  );
  const capacity = people.reduce(
    (sum, person) =>
      sum +
      weeks.reduce(
        (total, week) =>
          total + weekCapacity(person.weeklyHours, week.workdays),
        0,
      ),
    0,
  );
  const overallocated = people.filter((person) =>
    person.allocations.some(
      (hours, index) =>
        hours > weekCapacity(person.weeklyHours, weeks[index].workdays),
    ),
  ).length;
  const available = people.reduce(
    (sum, person) =>
      sum +
      weeks.reduce(
        (total, week, index) =>
          ...
          Math.max(
            0,
            weekCapacity(person.weeklyHours, week.workdays) -
              person.allocations[index],
          ),
        0,
      ),
    0,
  ...
  function saveSettings(next: Settings) {
    ...
    try {
      ...
      setStorageError(
        "Browser storage is unavailable. Settings will reset when you reload.",
      );
    }
  }
    ...
    <>
      <Box component="header" className="app-header">
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <ViewWeekOutlined sx={{ color: "primary.main", fontSize: 28 }} />
          <Typography sx={{ fontWeight: 700, fontSize: 20 }}>
            capacity<span className="brand-dot">.</span>
          </Typography>
      ...
      <Box component="main" className="capacity-page">
        <Box className="page-title-row">
          <Box>
            <Typography className="eyebrow">TEAM PLANNING</Typography>
            <Typography
              component="h1"
              sx={{
                fontWeight: 700,
        ...
        <CapacityControls
          ...
          onRefresh={() => {
            if (!mutation.isPending) void query.refetch();
          }}
        />
        <Box
          component="section"
        ...
        >
          {[
            ...
            },
            {
              ...
              value: `${formatHours(allocated)}h`,
              detail: `${formatHours(capacity)}h capacity`,
              tone: "text.primary",
            },
            {
            ...
            },
            {
              ...
              detail: "Above capacity in any week",
              tone: overallocated ? "error.main" : "text.primary",
            },
          ].map((metric) => (
            <Box key={metric.label} className="summary-item">
              <Typography
                sx={{ color: "text.secondary", fontSize: 12, mb: 0.75 }}
              >
                {metric.label}
              </Typography>
              <Typography
          ...
          >
            {mutation.isPending
              ...
              : query.isFetching
                ...
                : query.isError
                  ? "Refresh failed"
                  : `${people.length} people · ${weeks.length} ${weeks.length === 1 ? "week" : "weeks"}`}
          </Typography>
        </Box>
        <Box sx={{ height: 2 }}>
          {query.isFetching && <LinearProgress sx={{ height: 2 }} />}
        </Box>
        {query.isError && (
          <Alert
            severity="error"
        ...
        <Box component="footer" className="grid-footer">
          <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
            {[
              { color: "#217463", label: "Available" },
              { color: "#90723c", label: "At capacity" },
              { color: "#b83b43", label: "Overallocated" },
            ].map((item) => (
              <Box
                key={item.label}
                sx={{ display: "flex", alignItems: "center", gap: 0.75 }}
              >
                <Circle sx={{ fontSize: 7, color: item.color }} />
          ...
          <Typography sx={{ fontSize: 11, color: "text.secondary" }}>
            Monday - Friday · Hours per selected week
          </Typography>
        </Box>
      </Box>
        ...
        {editingPerson && (
          <WeeklyHoursEditor
            ...
            onSave={async (person) => {
              ...
              setNotice(`${person.name}'s capacity updated`);
            }}
          />
        )}
      </Suspense>
